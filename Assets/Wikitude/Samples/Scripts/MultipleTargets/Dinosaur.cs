...
public class Dinosaur : MonoBehaviour
{
    ...
    private const float DistanceThreshold = 0.06f;
    ...
    private IEnumerator RotateTowards(Transform rotationTarget) {
        ...
        var targetRotation = Quaternion.LookRotation((rotationTarget.position - transform.position).normalized, rotationTarget.up);
        ...
        if (angleToTarget > AngleThreshold) {
            ...
            while (angleToTarget > AngleThreshold && rotationTarget != null) {
                ...
                targetRotation = Quaternion.LookRotation((rotationTarget.position - transform.position).normalized, rotationTarget.up);
    ...
    private IEnumerator MoveTowards(Transform moveTarget, float distanceThreshold = DistanceThreshold) {
        ...
        if (distanceToTarget > DistanceThreshold) {
            ...
            while (distanceToTarget > distanceThreshold && moveTarget != null) {
                transform.LookAt(moveTarget, moveTarget.up);
